...
void FUnLuaEditorToolbar::CreateLuaTemplate_Executed()
{
    ...
    for (auto TemplateClass = Class; TemplateClass; TemplateClass = TemplateClass->GetSuperClass())
    {
        ...
        {
            const FString SuperName = Class->GetSuperClass()->GetName();
            if (SuperName == "Dialog" || SuperName == "EPanel" || SuperName == "CoreUI")
            {
                FullFilePath = BaseDir + "/Config/LuaTemplates/CoreUITemplate.lua";
            }
        }
        else if (Class->IsChildOf(UBTService::StaticClass()))
        {
            FullFilePath = BaseDir + TEXT("/Config/LuaTemplates/BTServiceTemplate.lua");
        }
        else if (Class->IsChildOf(UBTTaskNode::StaticClass()))
        {
            FullFilePath = BaseDir + TEXT("/Config/LuaTemplates/BTTaskTemplate.lua");
        }
        else if (Class->IsChildOf(UBTDecorator::StaticClass()))
        {
            FullFilePath = BaseDir + TEXT("/Config/LuaTemplates/BTDecoratorTemplate.lua");
        }
        ...
        //替换日期等信息
        Content = Content.Replace(TEXT("@COMPANY**"), TEXT("@ShenZhen EOneGame Co,Ltd."));
        const FDateTime Now = FDateTime::Now();
        const FString NowStr = Now.ToString(TEXT("%Y.%m.%d"));
        const FString NowTimeStr = Now.ToString(TEXT("%H:%M:%S"));
        Content = Content.Replace(TEXT("${date}"), *NowStr);
        Content = Content.Replace(TEXT("${time}"), *NowTimeStr);
