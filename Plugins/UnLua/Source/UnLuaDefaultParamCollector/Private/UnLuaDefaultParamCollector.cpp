...
public:
    ...
    virtual void Initialize(const FString& RootLocalPath, const FString& RootBuildPath, const FString& OutputDirectory, const FString& IncludeBase) override
    {
        ...
        //LV
        GeneratedFileContent += FString::Printf(TEXT("FInt64ParamValue* SharedInt64_Zero = new FInt64ParamValue(0);\r\n"));
        ...
        for (TFieldIterator<UFunction> FuncIt(Class, EFieldIteratorFlags::ExcludeSuper, EFieldIteratorFlags::ExcludeDeprecated); FuncIt; ++FuncIt)
        {
            ...
            for (TFieldIterator<FProperty> It(Function); It && (It->HasAnyPropertyFlags(CPF_Parm) && !It->HasAnyPropertyFlags(CPF_ReturnParm)); ++It)
            {
                ...
                else
                {
                    ...
                    //Add by LV int64 default param export to lua
                    else if (Property->IsA(FInt64Property::StaticClass())) // Int64
                    {
                        int64 Value = FCString::Atoi64(*ValueStr);
                        PreAddProperty(Class, Function);
                        if (Value == 0)
                            GeneratedFileContent += FString::Printf(TEXT("PC->Parameters.Add(TEXT(\"%s\"), SharedInt64_Zero);\r\n"), *Property->GetName());
                        else
                            GeneratedFileContent += FString::Printf(TEXT("PC->Parameters.Add(TEXT(\"%s\"), new FInt64ParamValue(%d));\r\n"), *Property->GetName(), Value);
                    } 
